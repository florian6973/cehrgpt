        ...
        while True:
            ...
            try:
                ...
                if char:
                    ...
                    sys.stdout.flush()
                    output.append(char)
                    if " 10/" in ''.join(output) or " 12/" in ''.join(output) or " 11/" in ''.join(output):
...
for depth in range(2, 16, 2):
    if depth != 2:
        for nhm in range(1, 86, 6):
            data.append((depth, 24 * nhm, 8))
    for nhm in range(43, 64, 4):
        data.append((depth, 48 * nhm, 16))

# for (nh, hs, nheads) in [(28, 2304, 18)]: # 1792 should be a multiple of 3 and num_heads

# results =  {}
results_df = pd.DataFrame(columns=["nh", "hs", "nheads", "n_parameters"])
# if os.path.exists("results_exact_values.csv"):
#     results_df = pd.read_csv("results_exact_values.csv")

for (nh, hs, nheads) in data:
    ...
    results_df.loc[len(results_df)] = (nh, hs, nheads, int(n_parameters))

    results_df.to_csv("results_exact_values.csv", index=False)
