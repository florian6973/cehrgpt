...
# for (nh, hsm) in [(14, 1), (14, 2), (14, 4), (20, 1), (20, 2), (20, 4), (28, 1), (28, 2), (28, 4), (32, 1), (32, 2), (32, 4), (28, 5), (44, 4)]:
    ...
    # Start VRAM monitoring before running the subprocess
    monitor_thread = None
    measurements = []
    
    if NVML_AVAILABLE:
        print(f"Starting VRAM monitoring for GPU {gpu_id} using pynvml...")
        monitor_thread, measurements = monitor_vram_async(gpu_id=gpu_id, interval=2.0)
    else:
        print(f"Starting VRAM monitoring for GPU {gpu_id} using nvidia-smi...")
        monitor_thread, measurements = monitor_vram_nvidia_smi_async(gpu_id=gpu_id, interval=2.0)
    ...
    if return_code == -1 or return_code == -5:
        print(f"Training run timed out after {args.timeout_seconds} seconds")
        sizes[(nh, hsm)] = "TIMEOUT"

    if monitor_thread and return_code == 1:
        vram_stats[(nh, hsm)] = {
            "max_used_mb": np.nan,
            "avg_used_mb": np.nan,
            "total_mb": np.nan,
    ...
    # if return_code != -1 and return_code != -5:  # Only extract parameters if process didn't timeout
    if True:
        # print(result_text)
        # exit()
        # n_parameters = result_text.strip().split('\n')[-1]
        txt = "Number of parameters:"
        n_parameters = result_text.strip().split(txt)[1].split(',')[0]
        print(f"Number of parameters: {n_parameters}")
        sizes[(nh, hsm)] = n_parameters

print("\n" + "="*50)
print("MODEL SIZE RESULTS:")
print("="*50)
pprint.pprint(sizes)
...
if vram_stats:
    ...
    print("="*50)
    for (nh, hsm), stats in vram_stats.items():
        print(f"Layers: {nh}, Hidden Size Multiplier: {hsm}")
...
vram_df.to_csv("vram_stats.csv", index=False)